    ...
    <div className='py-6 px-4'>
      ...
      <div>
        <p className='text-lg font-medium text-gray-800 mb-4'>
          Tổng chi tiêu: {totalPayments
            ? totalPayments.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })
            : '0 ₫'}
        </p>
            ...
            <li key={index}>
              ...
              <div>
                <span className='text-gray-600'>
                  Giá: {item.amount.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}
                </span>
              ...
              <div className='flex md-flex-row items-center space-x-2'>
                ...
                <p className='text-gray-600'>
                  Trạng thái: <span className={`ml-2 px-[2px] px-2 text-sm rounded ${item?.status === 'completed' ? 'bg-green-100 text-green-700' : item?.status === 'pending' ? 'bg-red-100 text-red-700' : item?.status === 'processing' ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-200 text-blue-700'}`}>{item?.status}</span>
                </p>
              </div>
              <hr className='my-2' />
            </li>
