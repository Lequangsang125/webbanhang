...
const UserPayments = () => {
  ...
  return (
    <div className='py-6 px-4'>
      ...
      <div>
        <p className='text-lg font-medium text-gray-800 mb-4'>Total Spent: ${totalPayments ? totalPayments : 0}</p>
        <ul>
          {
            orders && orders.map((item, index) => (
              <li key={index}>
                ...
                <div className='flex md-flex-row items-center space-x-2'>
                  <span className='text-gray-600'>Order Date: {new Date(item.createdAt).toLocaleString()}</span>
